...
pred.plot = ggplot(data=for.plot, aes(x=x, y=fit, ymin=lwr, ymax=upr))+
 ...
 ylab('Total relative citations')+
 coord_cartesian(ylim=c(-0.12, 4.5))+
 theme(panel.grid.minor = element_blank())+
...
## Change SD to "Standard deviation"
to.plot$type[to.plot$type=='SD'] = 'Standard deviation'
...
#  scale_color_manual('With highest cited\napplication', values=cbbPalette[2:3], labels=c('Yes','No'))+
  xlab('Application score statistic')+
  ylab('Inter-quartile range in total relative citations')+
  facet_wrap(~type, scales='free_x')+
  theme_bw()+
  # legend.title.align = 1, legend.text.align = 1, legend.position = c(0.82,0.8), 
  theme(panel.grid.minor = element_blank())
wplot
jpeg('BarnettScatterIQR.jpg', quality=100, units='in', width=5, height=3.2, res=400)
print(wplot)
invisible(dev.off())
```
